<script lang="ts">
    ...
    async function proceedCheckout() {
        ...
        openModal('Success', content.message);
    ...
    async function loadItems() {
        ...
        if (response.status !== 200) {
            openModal('Warning', 'You are not logged in');
            return;
        }
...
<div class="text-white p-6">
    ...
    <footer class="fixed inset-x-0 bottom-0 text-center bg-[#724c9d] p-4">
        <div class="place-content-evenly flex flex-row">
            <div class="flex flex-row items-center text-white">
                <div>Total Price: Rp. {numberFormat.format(totalPrice)}</div>
            </div>
            <Button on:click={proceedCheckout} class="w-auto" color="light">Checkout</Button>
        </div>
    </footer>
</div>
